import { Dir, readTextFile, writeFile, } from 'tauri/api/fs';
...
export async function getSettings(instanceUri) {
    const globalSettings = JSON.parse(await readTextFile(globalSettingsPath, { dir: Dir.App }));
    const instance = instanceUri ? JSON.parse(await readTextFile(instanceSettingsPath, { dir: Dir.App }))[instanceUri] : undefined;
    const ret = Object.assign({ "global": {
...
export async function setSetting(setting, value, instanceUri = undefined, database = undefined, resource = undefined) {
    if (instanceUri === undefined) {
        ...
        const blob = JSON.parse(await readTextFile(instanceSettingsPath, { dir: Dir.App }));
...
//# sourceMappingURL=tauri_db.js.map