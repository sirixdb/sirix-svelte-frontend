...
export function virtualize(node, { itemsCount, maxHeight, averageHeight = 24 }) {
    let startIndex = 0;
    let endIndex = 0;
    let topOffset = 0;
    let bottomOffset = 0;
    let heightMap = Array(itemsCount).fill(averageHeight);
    node.style.maxHeight = `${maxHeight}px`;
    async function handleScroll(event) {
        const oldStartIndex = startIndex;
        const oldEndIndex = endIndex;
        const { scrollTop } = node;
        let rows = Array.from(node.firstElementChild.children);
